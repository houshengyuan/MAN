...
def thread_function(nlp,WARNING,dataset,sent_id,word_dict,split_saving,lang):
    print(sent_id)
    ...
    #elif lang=="ja" or lang=="ch":
    #    word_list, bpe_info = remove_space_parse(nlp,raw_word_list)
    distance_list = []
...
def load_dataset_splits(task, splits, task_name):
    for split in splits:
        if split == 'train':
            task.load_dataset(split, combine=True,task=task_name,generate_distance=True)
        else:
            for k in itertools.count():
                split_k = split + (str(k) if k > 0 else '')
                try:
                    task.load_dataset(split_k, combine=False,task=task_name,generate_distance=True)
                except FileNotFoundError as e:
                    if k > 0:
                        break
                    raise e

def cal_distance_prior_main() -> None:
    parser = options.get_training_parser()
    args = options.parse_args_and_arch(parser)
    if args.max_tokens is None:
        args.max_tokens = 6000
    # ---------------- some hyper params ----------------#
    root = "/home/syhou/distance-transformer"
    corenlp_path = os.path.join(root, "data", "stanford-corenlp-full-2018-10-05")
    save_path = os.path.join(root, "experimental", "distance_prior",args.task_name)
    split = 'test'
    src_tgt = 'src'
    # ----------------  params domains ----------------#
    ...
    nlp = StanfordCoreNLP(corenlp_path, lang=lang_dict[args.source_lang])
    ...
    # Load dataset splits
    load_dataset_splits(task,[split], args.task_name)

    word_dict = task.datasets[split].src_dict if src_tgt == 'src' else task.datasets[split].tgt_dict

    WARNING = 0
    #poolsize=300
    #pool = multiprocessing.Pool(processes = poolsize)
    #[pool.apply_async(thread_function, (nlp, WARNING, dataset, sent_id, word_dict, split_saving,cfg.task.source_lang)) for i, sent_id in enumerate(tqdm(range(dataset.__len__()))) if i>=0 and i<=100000]
    # from 150 interval-15 xia-10 from 300 interval 10
    for i, sent_id in enumerate(tqdm(range(task.datasets[split].__len__()))):
       if sent_id>=0 and sent_id<=100000:
        thread_function(nlp, WARNING, task.datasets[split], sent_id, word_dict, split_saving,args.source_lang)
    #pool.close()
    #pool.join()
